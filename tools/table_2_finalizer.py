...
def table_2_finalizer(
    ...
    benchmark_sizes = pd.read_csv(
        experiment_dir_path / "benchmark_sizes.csv", delimiter=','
    )
    benchmark_sizes = benchmark_sizes.rename(columns={"size": "size"})

    benchmark_r3_sizes = pd.read_csv(
        experiment_dir_path / "benchmark_r3_sizes.csv", delimiter=','
    )
    benchmark_r3_sizes = benchmark_r3_sizes.rename(columns={"size": "size_r3"})

    benchmark_r6_sizes = pd.read_csv(
        experiment_dir_path / "benchmark_r6_sizes.csv", delimiter=','
    )
    benchmark_r6_sizes = benchmark_r6_sizes.rename(columns={"size": "size_r6"})

    benchmark_r2_s_sizes = pd.read_csv(
        experiment_dir_path / "benchmark_r2_s_sizes.csv", delimiter=','
    )
    benchmark_r2_s_sizes = benchmark_r2_s_sizes.rename(columns={"size": "size_r2_s"})

    benchmark_r6_s_sizes = pd.read_csv(
        experiment_dir_path / "benchmark_r6_s_sizes.csv", delimiter=','
    )
    ...
    r_results_1_df = r_results_1_df.rename(
        columns={'Benchmark': 'circuit_name', 'resyn2_6 Time': 'time_r6'}
    )
    ...
    table_1_df['class'] = table_1_df['circuit_name'].map(
        lambda x: x.split('__')[0].replace('_sat', '').replace('_unsat', '')
    )
    table_1_df['absolute'] = (
        table_1_df['size_r6'] - table_1_df['size_r6_s']
    ) / table_1_df['size_r6']
    table_1_df['relative'] = (table_1_df['size_r6'] - table_1_df['size_r6_s']) / (
        table_1_df['size'] - table_1_df['size_r6']
    )
    table_1_df['time'] = (
        (table_1_df['time_r6_s'] + table_1_df['time_r6']) - table_1_df['time_r6']
    ) / table_1_df['time_r6']
    ...
    click.echo(result_1_df.to_string())
    ...
    r_results_2_df = r_results_2_df.rename(
        columns={
            'Benchmark': 'circuit_name',
            'resyn2_2 Time': 'time_r2',
            'resyn2_3 Time': 'time_r3',
        }
    )
    ...
    table_2_df['class'] = table_2_df['circuit_name'].map(
        lambda x: x.split('__')[0].replace('_sat', '').replace('_unsat', '')
    )
    table_2_df['absolute'] = (
        table_2_df['size_r3'] - table_2_df['size_r2_s']
    ) / table_2_df['size_r3']
    table_2_df['relative'] = (table_2_df['size_r3'] - table_2_df['size_r2_s']) / (
        table_2_df['size'] - table_2_df['size_r3']
    )
    table_2_df['time'] = (
        (table_2_df['time_r2_s'] + table_2_df['time_r3']) - table_2_df['time_r3']
    ) / table_2_df['time_r3']
    ...
    click.echo(result_2_df.to_string())
