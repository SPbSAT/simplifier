...
class IBenchParser: public ICircuitParser
{
  ...
  protected:
     ...
     */
    inline virtual void handleGate(
        std::string_view op,
        GateId gateId,
        GateIdContainer const& var_operands) = 0;
     ...
     * @param var_operands -- operands of gate.
     */
    inline virtual bool specialOperatorCallback_(
        GateId gateId,
        std::string_view op,
        std::string_view operands_str) = 0;
        ...
        else
        {
            ...
            if (specialOperatorCallback_(gateId, op, operands_str))
            {
                ...
                logger.debug(
                    ...
                    "Operator: \"", op, "\";\n",
                    "\tEncoded Name: \"", encoder.encodeGate(var_name), "\";\n",
                    "\tOperands str: \"", operands_str, "\"");
                #endif
    ...
    inline std::tuple<size_t, size_t, size_t> _getDelimitersPositions(std::string_view line)
    {
        ...
        if (
            ...
            || l_bkt_idx == std::string::npos
            || r_bkt_idx == std::string::npos
            || eq_idx >= l_bkt_idx
            || eq_idx >= r_bkt_idx
            || l_bkt_idx >= r_bkt_idx
            )
