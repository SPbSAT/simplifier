...
void printCircuit(csat::DAG const &circuit, csat::utils::GateEncoder<std::string> const &encoder) {
    ...
    for (size_t gateId = 0; gateId < circuit.getNumberOfGates(); ++gateId) {
        if (circuit.getGateType(gateId) != csat::GateType::INPUT) {
            ...
            std::cout << operands.at(num_operands - 1) << " => "
                    << encoder.decodeGate(operands.at(num_operands - 1)) << ")\n";
        }
    }
}
...
std::optional<std::ofstream> openFileStat(const argparse::ArgumentParser &program) {
...
void runBenchmark(const std::string &file_path, const argparse::ArgumentParser &program, csat::Logger &logger,
...
int main(int argn, char **argv) {
    ...
    program.add_argument("-o", "--output")
            .help("path to resulting directory");
    program.add_argument("-s", "--statistics")
            .metavar("FILE")
